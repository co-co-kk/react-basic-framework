...
const ComponentRenderer = React.memo(({ block }: { block: Block }) => {
...
const DraggableComponent = React.memo(({ id, children }: { id: string; children: React.ReactNode }) => {
...
const DragGhost = React.memo(({ block }: { block: Block }) => {
  return (
...
const layoutHelpers = (layout: Layout, setLayout: React.Dispatch<React.SetStateAction<Layout>>) => {
  const findBlock = React.useCallback((id: string) => {
  ...
  const removeBlockById = React.useCallback((id: string) => {
  ...
  const findRowIdByColId = React.useCallback((colId: string): string | null => {
    for (const r of layout) {
  ...
  const cleanup = React.useCallback((rows: Layout): Layout => {
    const cleaned = rows
  ...
  const normalizeRowWidths = React.useCallback((rows: Layout) =>
    rows.map((r) => {
  ...
  const emptyRow = React.useCallback((): Row => ({ id: newid("row"), columns: [{ id: newid("col"), widthPct: 100, blocks: [] }] }), []);

  // 插入/移动 API
  const insertBeforeBlock = React.useCallback((anchorBlockId: string, block: Block) => {
    setLayout((prev) =>
      cleanup(
        prev.map((r) => ({
          ...r,
  ...
  const insertAfterBlock = React.useCallback((anchorBlockId: string, block: Block) => {
    setLayout((prev) =>
      cleanup(
        prev.map((r) => ({
          ...r,
  ...
  const insertNewColumnBesideBlock = React.useCallback((anchorBlockId: string, side: "left" | "right", block: Block) => {
    setLayout((prev) =>
      cleanup(
        prev.map((r) => {
  ...
  const insertNewRowBesideBlock = React.useCallback((anchorBlockId: string, pos: "above" | "below", block: Block) => {
    setLayout((prev) => {
      const next: Layout = [];
      for (const r of prev) {
  ...
  const moveBlockToNewRowAt = React.useCallback((draggedId: string, rowId: string, pos: "above" | "below") => {
  ...
  }, [cleanup, removeBlockById, setLayout]);

  // 移动（非复制）：先移除，再按 action 插入
  const moveBlockToAction = React.useCallback((
    draggedId: string,
    action:
    ...
    // 添加防抖处理，避免频繁的状态更新
    if (moveBlockToAction.debounceTimer) {
      clearTimeout(moveBlockToAction.debounceTimer);
    }

    moveBlockToAction.debounceTimer = setTimeout(() => {
      const { removed, next } = removeBlockById(draggedId);
      if (!removed) return;
      setLayout(
        cleanup(
  ...
  // 为函数添加静态属性来保存定时器引用
  (moveBlockToAction as any).debounceTimer = null;

  const resize = React.useCallback((rowId: string, leftColId: string, deltaPct: number) => {
    setLayout((prev) =>
      prev.map((r) => {
...
/********************* 画布/行/列/块 *********************/
const Canvas = React.memo(({ layout, helpers }: { layout: Layout; helpers: ReturnType<typeof layoutHelpers> }) => {
  return (
...
const RowView = ({ row, helpers }: { row: Row; helpers: ReturnType<typeof layoutHelpers> }) => {
  ...
  // 使用useCallback优化函数定义，避免在每次渲染时重新创建
  const getAccumulatedSize = useCallback((index: number): number => {
    return colSizes.slice(0, index).reduce((acc, size) => acc + size, 0);
  }, [colSizes]);

  // 优化resize函数，避免过于频繁的调用
  const resizeDebounce = useRef<NodeJS.Timeout | null>(null);
  const debouncedResize = useCallback((rowId: string, leftColId: string, deltaPct: number) => {
    if (resizeDebounce.current) {
      clearTimeout(resizeDebounce.current);
    }
    resizeDebounce.current = setTimeout(() => {
      helpers.resize(rowId, leftColId, deltaPct);
    }, 16); // 约60fps
  }, [helpers]);
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    requestAnimationFrame(() => {
      ...
      // 防抖动调用resize函数，避免频繁更新布局
      debouncedResize(row.id, row.columns[leftColIndex].id, leftColNewSize - row.columns[leftColIndex].widthPct);
    });
  }, [isDragging, dragIndex, colSizes, getAccumulatedSize, debouncedResize, row.id, row.columns]);
...
const ColumnView = React.memo(({ rowId, col, helpers, widthPct }: { rowId: string; col: Column; helpers: ReturnType<typeof layoutHelpers>; widthPct?: number }) => {
...
/********************* 块 + 四边缘投放区 *********************/
const BlockCard = React.memo(({ block, rowId, colId, helpers }: { block: Block; rowId: string; colId: string; helpers: ReturnType<typeof layoutHelpers> }) => {
...
/********************* 工具栏 *********************/
const Toolbar = React.memo(({ onAddComponent, onReset }: { onAddComponent: (component: any) => void; onReset: () => void }) => {
  return (
...
const NewDndPage = () => {
  ...
  const onDragEnd = (e: DragEndEvent) => {
    ...
    // 使用requestAnimationFrame延迟处理，提升拖拽释放时的流畅度
    requestAnimationFrame(() => {
  ...
  return (
    <DndContext
      ...
      measuring={{
        droppable: {
          strategy: "requestAnimationFrame",
        }
      }}
    >
