  ...
  const [layout, setLayout] = useState<LayoutState>(() => [
    {
      id: newId("row"),
      columns: [
        { id: newId("col"), widthPct: 50, blocks: [ { id: newId("blk"), render: <DemoCard title="A" /> }, { id: newId("blk"), render: <DemoCard title="A2" /> } ] },
        { id: newId("col"), widthPct: 50, blocks: [ { id: newId("blk"), render: <DemoCard title="B" /> } ] },
      ],
    },
    {
      id: newId("row"),
      columns: [
        { id: newId("col"), widthPct: 100, blocks: [ { id: newId("blk"), render: <DemoCard title="C" /> } ] },
      ],
    },
  ]);
  ...
  return [
    {
      id: newId("row"),
      columns: [
        { id: newId("col"), widthPct: 33.34, blocks: [ { id: newId("blk"), render: <DemoCard title="X" /> } ] },
        { id: newId("col"), widthPct: 33.33, blocks: [ { id: newId("blk"), render: <DemoCard title="Y" /> }, { id: newId("blk"), render: <DemoCard title="Y2" /> } ] },
        { id: newId("col"), widthPct: 33.33, blocks: [ { id: newId("blk"), render: <DemoCard title="Z" /> } ] },
      ],
    },
  ];
}
...
function Canvas({ layout, onChange }: { layout: LayoutState; onChange: (l: LayoutState) => void; }) {
  const removeBlock = useCallback((blockId: string) => {
    ...
    onChange(cleaned.length ? cleaned : [ { id: newId("row"), columns: [ { id: newId("col"), widthPct: 100, blocks: [] } ] } ]);
  }, [layout, onChange]);
  ...
  const insertNewColumnBeside = useCallback((targetRowId: string, targetColId: string, side: "left" | "right", block: BlockNode) => {
    const next = layout.map(row => {
      ...
      // 新列默认宽度：从相邻列挤出 20%
      const newWidth = 20;
      const newCol: ColumnNode = { id: newId("col"), widthPct: newWidth, blocks: [block] };
...
function BlockCard({ block, rowId, colId, index, onDropAbove, onDropBelow, onDropLeft, onDropRight, onDropRowAbove, onDropRowBelow }: {
  ...
  return (
    <div ref={ref} className={`group relative mb-2 rounded-lg border border-slate-200 bg-white shadow-sm p-3 cursor-move select-none ${isDragging ? "opacity-50" : ""}`}>
      {/* 边缘可视化提示区 */}
